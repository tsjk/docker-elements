...
RUN	   eselect repository add bitcoin git https://gitlab.com/bitcoin/gentoo.git\
	...
	&& echo '>=net-p2p/elements-23.2.2' >> '/etc/portage/package.mask/net-p2p:elements'\
	&& mkdir -p /etc/portage/package.accept_keywords\
	&& echo '~net-p2p/elements-23.2.1::bitcoin **' >> '/etc/portage/package.accept_keywords/net-p2p:elements'\
